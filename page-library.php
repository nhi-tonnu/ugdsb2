 ...
 *
 * This is the template that displays Library pages with Library's specific widgets
 *
 * @package ugdsb2
 */
 ?>
...
<?php $has_left = FALSE; ?>
<?php $has_right = FALSE; ?>
<?php if (has_nav_menu('lmenu') || (is_active_sidebar('sidebar-library'))) {$has_left = TRUE;} ?>
...
<?php
    ...
    elseif (($has_left == TRUE) and ($has_right == FALSE)):
      ...
      get_sidebar('lmenu');//display left menu
      if (!is_front_page()) {
          get_sidebar('library');//display left menu
      }
      echo '</div>';
